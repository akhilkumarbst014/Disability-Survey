    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            const user = users.find(
                (user) =>
                    (user.Username === Username || user.Mobile === Username) && user.Password === Password
            );
            console.log(user);
            ...
            <div className='relative w-full md:w-1/2 h-1/2 md:h-full flex flex-col'>
                <div className='absolute top-1/2 md:top-[55%] transform -translate-y-1/2 md:transform-none flex flex-col'>
                    <h1 className='text-2xl md:text-4xl text-amber-100 font-bold my-3 mx-4 text-left'>
                        SURVEY OF <br />PERSON<br /> WITH DISABILITIES
                    </h1>
            ...
            <div className='w-full  md:w-1/2 h-full bg-[#f5f5f5] flex flex-col p-10 md:pt-32 justify-between items-center'>
                <div className='w-full flex flex-col max-w-[580px]'>
                    <div className='w-full flex flex-col mb-4 mt-1'>
                        ...
                        <p className='text-base mb-2 font-semibold '>Welcome Back! Please enter your details.</p>
                    </div>
                    ...
                    <div className='w-full flex items-center justify-end'>
                        <p className='text-sm font-medium whitespace-nowrap cursor-pointer underline underline-offset-2 mt-1'>Forget Password</p>
                    </div>
                    {error && <p style={{ color: "red" }}>{error}</p>}
                    <div className='w-full flex flex-col my-4'>
                        <button type='submit' className='w-full text-white my-2 font-semibold bg-[#060606] rounded-md p-2 text-center flex items-center justify-center cursor-pointer' onClick={handleLogin}>
                            Log in
                        </button>
